...
import type { CardMeta, DeckSection } from "@sorcery/types";
import { imageUrl, MIN_ATLAS, MIN_SPELLBOOK, validateDeck } from "@sorcery/types";
...
function SectionList({
  ...
  return (
    <div className="deck-section">
      <h4>
        {title}{" "}
        <span className={count >= min ? "ok" : "warn"}>
          {count}/{min}
        </span>
      </h4>
      {rows.length === 0 ? (
        <p className="muted small">Empty — add cards from their detail view.</p>
      ) : (
    ...
    <aside className="deckpanel">
          ...
          <ul className="deck-list">
            {list.map((d) => {
              ...
              <li key={d.id} className="deck-list-item">
                <button className="deck-open" onClick={() => void openDeck(d.id)}>
                  <span className="deck-thumb">
                    {slug ? (
                      <img src={imageUrl(slug)} alt={d.avatar ?? ""} loading="lazy" />
                    ) : (
      ...
      {active && validation && (
        <div className="deck-editor">
          ...
          <div className={`deck-status ${validation.valid ? "valid" : "invalid"}`}>
            {validation.valid ? (
              <strong>✓ Legal deck</strong>
            ) : (
              <ul>
